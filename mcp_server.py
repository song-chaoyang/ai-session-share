...
即可在不离开对话的情况下:
  ...
  - list_sessions   列出所有活动会话(托管/tmux/Claude/atomcode/共享服务)与状态
...
def t_list_sessions(args):
    ...
    mg = s.get("managed", [])
    if mg:
        lines.append("托管会话(免 tmux,网页 /w/<id>,生命周期随进程):")
        for x in mg:
...
TOOLS = [
    {
        "name": "list_sessions",
        "description": "列出本机所有活动会话与状态:托管会话(免 tmux)、ttyd 共享终端、Claude Code 会话、atomcode 活动。",
